...
class DNDetailViewController: UIViewController {

    var model: DNNoteModelSwift?
    ...
    @objc func rightItemTarget() {
        ...
        DNNoteAlertTool.alertTool.alertForMessage("是否更新", self, {
            
            let model = DNNoteModelSwift()
            model.content = self.textView?.text
            model.dayDate = self.model?.dayDate
            model.timeDate = self.model?.timeDate
            model.timeline = self.getCurrentTimeInterval()
            
            DNDataManager.manager.updateData(model: model, uid: self.model!.user_fmdb_id!, successHandler: {
